...
namespace SolOraiRelayer {
  const interval = ITERATION_DELAY.SOL_TO_ORAI_BRIDGE_DELAY;
  ...
  export type GravityRelayerContext = StandardRelayerContext &
    AttestationContext;

  const listenVaa = async () => {
    const app = new StandardRelayerApp<GravityRelayerContext>(
      Environment.MAINNET,
      {
        name: "GravityRelayer",
        logger: logger("GravityRelayer"),
        spyEndpoint: envConfig.wormhole.spyEndpoint,
        redis: {
        ...
        missedVaaOptions: {
          startingSequenceConfig: {
            1: BigInt(envConfig.wormhole.startSequence || 0),
          },
        },
      }
    );
        ...
        if (!isTransferCompleted) {
          ...
          if (isSolanaToBsc) {
            ...
            if (toAddress === walletAddress) {
              ...
              if (allowance.toBigInt() < BigInt(denormalizedAmount)) {
                ...
                handleQueueProcessLogger.info(
                  `Approve 9999999999999999999999999 ${tokenWrappedAddress} txHash: ${txApproveReceipt.transactionHash}`
                );
              }
              ...
              handleQueueProcessLogger.info(
                `Send ${deNormalizeAmount} ${tokenWrappedAddress} to ${oraiWallet} Cosmos txHash: ${txSendToCosmosReceipt.transactionHash}`
              );
            }
          }
        }
  ...
  export const start = async () => {
    await Promise.all([listenVaa(), handleVaa()]);
  };
}

export default SolOraiRelayer;
